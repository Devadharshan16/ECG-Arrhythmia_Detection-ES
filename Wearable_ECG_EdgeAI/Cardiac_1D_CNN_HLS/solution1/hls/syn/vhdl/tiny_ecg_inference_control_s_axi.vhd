...
entity tiny_ecg_inference_control_s_axi is
generic (
...
port (
    ...
    RREADY                :in   STD_LOGIC;
    input_ecg             :out  STD_LOGIC_VECTOR(63 downto 0);
    output_logits         :out  STD_LOGIC_VECTOR(63 downto 0)
);
end entity tiny_ecg_inference_control_s_axi;
...
architecture behave of tiny_ecg_inference_control_s_axi is
...
attribute DowngradeIPIdentifiedWarnings of behave : architecture is "yes";
    ...
    constant ADDR_INPUT_ECG_DATA_0     : INTEGER := 16#10#;
    constant ADDR_INPUT_ECG_DATA_1     : INTEGER := 16#14#;
    constant ADDR_INPUT_ECG_CTRL       : INTEGER := 16#18#;
    ...
    -- internal registers
    signal int_input_ecg       : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_INPUT_ECG_DATA_0 =>
                        rdata_data <= RESIZE(int_input_ecg(31 downto 0), 32);
                    when ADDR_INPUT_ECG_DATA_1 =>
                        rdata_data <= RESIZE(int_input_ecg(63 downto 32), 32);
...
-- ----------------------- Register logic ----------------
    input_ecg            <= STD_LOGIC_VECTOR(int_input_ecg);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_input_ecg(31 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_INPUT_ECG_DATA_0) then
                    int_input_ecg(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_input_ecg(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_input_ecg(63 downto 32) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_INPUT_ECG_DATA_1) then
                    int_input_ecg(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_input_ecg(63 downto 32));
                end if;
            end if;
        end if;
    end process;
