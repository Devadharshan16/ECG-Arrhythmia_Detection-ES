...
// ==============================================================
`timescale 1ns/1ps
(* DowngradeIPIdentifiedWarnings="yes" *) module tiny_ecg_inference_control_s_axi
#(parameter
...
//------------------------Parameter----------------------
localparam
    ADDR_INPUT_ECG_DATA_0     = 6'h10,
    ADDR_INPUT_ECG_DATA_1     = 6'h14,
    ADDR_INPUT_ECG_CTRL       = 6'h18,
    ADDR_OUTPUT_LOGITS_DATA_0 = 6'h1c,
    ADDR_OUTPUT_LOGITS_DATA_1 = 6'h20,
    ADDR_OUTPUT_LOGITS_CTRL   = 6'h24,
    WRIDLE                    = 2'd0,
    WRDATA                    = 2'd1,
    WRRESP                    = 2'd2,
    WRRESET                   = 2'd3,
...
always @(posedge ACLK) begin
    if (ACLK_EN) begin
        if (ar_hs) begin
            rdata <= 'b0;
            case (raddr)
                ADDR_INPUT_ECG_DATA_0: begin
                    rdata <= int_input_ecg[31:0];
                end
                ADDR_INPUT_ECG_DATA_1: begin
                    rdata <= int_input_ecg[63:32];
                end
...
//------------------------Register logic-----------------
assign input_ecg     = int_input_ecg;
assign output_logits = int_output_logits;
// int_input_ecg[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_input_ecg[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_INPUT_ECG_DATA_0)
            int_input_ecg[31:0] <= (WDATA[31:0] & wmask) | (int_input_ecg[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_input_ecg[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_INPUT_ECG_DATA_1)
            int_input_ecg[63:32] <= (WDATA[31:0] & wmask) | (int_input_ecg[63:32] & ~wmask);
    end
end
