...
def simulate_hls_corrupted_dataset(x_raw):
    from parity_test import p1, p2, p3, p4, input_scale, input_zp, hls_conv1d, hls_conv1d_2d
    x_np = x_raw.squeeze(0).numpy()
    
    # OLD extract_ds2_samples logic (clipped to 127)
    corrupted_int8 = np.clip(np.round(x_np / input_scale) + input_zp, -128, 127).astype(np.int8)
    
    # Cast to uint8 exactly as helloworld.c does: input_ecg[i] = ds2_all_samples[i]
    hls_input = corrupted_int8.astype(np.uint8)
    ...
    # Perfect FC logic (uint8, clamp 255)
...
print(f"{'Idx':<8} | {'Corrupted DS Output':<20} | HW Pred")

matches = 0
for idx in indices:
    x_raw, _ = dataset[idx]
    
    out = simulate_hls_corrupted_dataset(x_raw)
